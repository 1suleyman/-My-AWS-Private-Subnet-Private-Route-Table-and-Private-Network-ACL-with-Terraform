...
# 5. Create Public and Private Route Tables and associate it with their respective Subnets
...
# 7. Create a Network ACL, rules and associate it with the Public Subnet
resource "aws_network_acl" "public_subnet_acl" {
  ...
  ingress {
    ...
    rule_no     = var.public_network_acl_inbound_rule_number
    action      = var.public_network_acl_inbound_rule_action
  ...
  egress {
    ...
    to_port     = 0
    protocol    = "-1" # All protocols
    rule_no     = var.public_network_acl_outbound_rule_number
    action      = var.public_network_acl_outbound_rule_action
    cidr_block  = var.internet_cidr_block
  }
...
# 8. Create a Network ACL, rules and associate it with the Private Subnet
...
# custom NACL starts as deny all so no need to add deny rules
